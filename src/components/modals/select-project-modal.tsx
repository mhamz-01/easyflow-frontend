...
const SelectProjectModal = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="bg-[#1C1C1C] border border-neutral-800 text-white sm:max-w-md p-0 overflow-hidden">
        <DialogHeader className="px-6 pt-6 pb-2">
          <div className="flex items-center gap-3">
            <span className="p-2 rounded-lg  border border-neutral-700">
        ...
        <div className="px-3 pb-3 max-h-[320px] overflow-y-auto">
          ...
          {!isLoading && projects.length === 0 && (
            <div className="flex flex-col items-center text-center gap-3 py-10 px-6">
              <span className="p-3 rounded-full bg-[#0F0F0F] border border-neutral-800">
              ...
              <Dialog onOpenChange={(open) => open && openModal("isCreateProjectModalOpen")}>
                <DialogTrigger asChild>
                  <button className="mt-1 text-sm font-medium px-4 py-2 rounded-lg bg-white text-black hover:bg-neutral-200 transition-colors">
                    Create project
                  </button>
                </DialogTrigger>
                <CreateProjectModal />
            ...
            <>
              {projects.map((project) => (
                <button
                  ...
                  className="w-full flex items-center justify-between gap-3 px-4 py-3 rounded-lg hover:bg-[#262626] transition-colors text-left group"
                >
                  <span className="text-sm font-medium truncate">{project.name}</span>
                  <ArrowRight
                    size={15}
                    className={`text-neutral-500 transition-transform duration-200 ${
                      hoveredId === project.id ? "translate-x-0.5 text-neutral-300" : ""
                    }`}
                  />
                </button>
              ))}
              <Dialog onOpenChange={(open) => open && openModal("isCreateProjectModalOpen")}>
                <DialogTrigger asChild>
                  <button className="w-full flex items-center gap-2 px-4 py-3 mt-1 rounded-lg hover:bg-[#262626] transition-colors text-left text-neutral-400 hover:text-white">
                    <Plus size={15} />
