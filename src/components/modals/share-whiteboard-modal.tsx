...
import { assignWhiteboard } from "@/src/lib/api/whiteboards/services";
...
const ShareWhiteboardModal = ({
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: () => assignWhiteboard({ whiteboardId, memberIds: selectedIds }),
    onSuccess: () => {
      toast.success("Whiteboard shared successfully");
      queryClient.invalidateQueries({ queryKey: ["whiteboards"] });
      onClose();
    },
    onError: () => toast.error("Failed to share whiteboard"),
  });
    ...
    <DialogContent className="w-full max-w-sm sm:max-w-md">
      <DialogHeader>
        ...
        <DialogDescription>
          ...
          <span className="font-medium">"{whiteboardName}"</span>. They'll
