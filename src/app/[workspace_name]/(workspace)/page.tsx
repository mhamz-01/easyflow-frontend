...
export default function Home() {
  ...
  const [selectedAction, setSelectedAction] = useState<
    (typeof homePageActions)[number] | null
  >(null);
      ...
      <section className="grid place-content-center pb-6">
        <div>
          <div className="text-center max-w-xl mx-auto">
            <span className="inline-block text-xs font-medium tracking-wide uppercase text-primary-blue mb-2">
              {greeting}, {firstName}
            </span>
          ...
          <div className="flex items-center gap-5 justify-center flex-wrap mt-10">
            {homePageActions.map((action) => {
              const color = actionColors[action.name] ?? "#2563eb";
              ...
              return (
                <div
                  ...
                  className="group relative flex flex-col items-center text-center bg-[linear-gradient(to_bottom,#1C1C1C_0%,#1C1C1C_60%,#0F0F0F_100%)] p-6 rounded-2xl border border-white/[0.04] transition-all duration-300 ease-out hover:cursor-pointer min-w-[205px]"
                >
                  <span
                    className="absolute inset-0 rounded-2xl transition-colors duration-300 pointer-events-none"
                    style={
                      isHovered
                  ...
                  <span className="flex items-center justify-center w-12 h-12 rounded-xl transition-colors duration-300">
                    <Image
                      src={action.icon}
                      alt={action.name}
                      width={120}
